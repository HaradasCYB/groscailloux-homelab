...
JELLYFIN_PUBLIC_URL="${JELLYFIN_PUBLIC_URL:-https://jellyfin.example.com}"
API_KEY="${JELLYFIN_API_KEY:?missing JELLYFIN_API_KEY}"

# Jellyfin library IDs — fournir dans .env apres init Jellyfin
# (Settings > Libraries > inspect each library to get the GUID)
LIB_FILMS="${JELLYFIN_LIB_FILMS:?missing JELLYFIN_LIB_FILMS}"
LIB_SERIES="${JELLYFIN_LIB_SERIES:?missing JELLYFIN_LIB_SERIES}"
...
→ Pour Jellyseerr, l'utilisateur clique l'onglet "Use your Jellyfin account"
  sur ${JELLYSEERR_PUBLIC_URL:-https://jellyseerr.example.com}/login
